...
def model_to_dict(instance, recurse=True, backrefs=False, only=None, exclude=None):
    """
    Convert a model instance to a dictionary.

    :param instance: Model instance to convert.
    :param recurse: Whether to convert related objects.
    :param backrefs: Whether to convert reverse foreign key references.
    :param only: Whitelist of fields to include in the dictionary.
    :param exclude: Blacklist of fields to exclude from the dictionary.
    ...
    for field_name, field_obj in model_class._meta.fields.items():
        ...
        if value is not None:
            if isinstance(field_obj, ForeignKeyField) and recurse:
                if backrefs:
                    related_name = field_obj.backref or 'set'
                    value = [model_to_dict(rel, backrefs=False) for rel in getattr(instance, related_name)]
                else:
                    value = model_to_dict(value, backrefs=False)
